...
if (interaction.customId === "hide") {
      ...
      if (interaction.customId === "lock") {
        ...
        perms.unshift({ id: interaction.guild.id, allow: ['ViewChannel'], deny: ['Connect'] }); // เห็นแต่เข้าไม่ได้
        await channel.permissionOverwrites.set(perms);
        return interaction.editReply("🔒 ล็อคห้องเรียบร้อยแล้ว");
      }

      if (interaction.customId === "unlock") {
        const perms = getBasePerms();
        perms.unshift({ id: interaction.guild.id, allow: ['ViewChannel', 'Connect'] });
        await channel.permissionOverwrites.set(perms);
        return interaction.editReply("🔓 ปลดล็อคห้องเรียบร้อยแล้ว");
      }
      
      // ... (โค้ดส่วนอื่นของคุณ เช่น name, limit, allow, deny, transfer ให้วางต่อได้เลย)
      }
    }
