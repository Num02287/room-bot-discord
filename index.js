...
client.on("voiceStateUpdate", async (oldState, newState) => {
  try {
    ...
    if (newState.channelId === createChannelId) {
      ...
      // ===== เพิ่มสถานะ locked โดยไม่แก้ระบบอื่น =====
      ...
      if (interaction.customId === "show") {
        ...
        await channel.permissionOverwrites.edit(
          interaction.guild.id,
          {
            ...
            // ===== เชื่อมกับ 🔒 / 🔓 =====
            // ถ้าล็อกอยู่ = Connect false
            // ถ้าไม่ล็อก = Connect true
            Connect: data.locked ? false : true
          }
        );

        // ตั้งค่าสิทธิ์ยศพิเศษตามสถานะล็อก
        if (allowRoleId) {
          await channel.permissionOverwrites.edit(
            allowRoleId,
            {
        ...
        return interaction.editReply({
          content: data.locked
            ? "👁️ แสดงห้องเรียบร้อยแล้ว แต่ห้องยัง 🔒 ล็อกอยู่"
            : "👁️ แสดงห้องเรียบร้อยแล้ว ห้องอยู่ในสถานะ 🔓 ไม่ล็อก"
        });
      }
    }
