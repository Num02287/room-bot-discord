...
client.on("voiceStateUpdate", async (oldState, newState) => {
  ...
  if (oldState.channelId && tempChannels.has(oldState.channelId)) {
    ...
    if (oldState.member.id === data.owner) {
      ...
      channel
        .setName(`📍・ห้องส่วนตัวของ ${newOwner.user.username}`)
        .catch(() => {});
    }
  }
});
  ...
  try {
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId === "hide") {
        ...
        if (allowRoleId)
          await channel.permissionOverwrites
            .edit(allowRoleId, { ViewChannel: false })
            .catch(() => {});
      ...
      if (interaction.customId === "show") {
        ...
        if (allowRoleId)
          await channel.permissionOverwrites
            .edit(allowRoleId, { ViewChannel: true, Connect: false })
            .catch(() => {});
    ...
    if (interaction.isUserSelectMenu()) {
      ...
      if (!channel || !data || data.owner !== interaction.member.id)
        return interaction.reply({ content: "❌ ผิดพลาด", ephemeral: true });
